...
const profiles=(process.env.PROFILES||'legacy,previous,launch').split(','),shots=process.argv.includes('--shots'),only=process.env.CASE;
...
const open=async(profile,single)=>{await page.goto(`${base}?fourWallPerf&launchProbe&groupingProbe&physics=${profile==='previous'?'launch':profile}&grouping=${profile==='previous'?'previous':'individual'}${single?'&camera=default':''}`);await page.waitForFunction(()=>window.aquarium?.elapsed>2,null,{timeout:90000});};
const four=async()=>{
...
try{
 for(const profile of profiles) {
  const dir=`verify/grouping-${profile==='legacy'?'before':profile==='launch'?'after':'current'}`;await mkdir(dir,{recursive:true});
  if(!shots)for(const scenario of (only?[only]:['single','four'])) {
   ...
   const data=await page.evaluate(()=>({launch:window.launchProbe.launch,frames:window.launchProbe.frames,perf:window.fourWallPerf.samples,physics:window.aquarium.physics,grouping:window.groupingProbe.snapshot()}));
   ...
   const row={profile,scenario,grouping:data.grouping,impulseFrames:data.launch.impulseFrames,impulses:data.launch.impulses,stepsFirstHalfSecond:closest(.5,'wallAge').totalSteps,stepsFirstHalfSimSecond:closest(.5).totalSteps,fpsAirborne:fps(.1,1.5),fpsSettled:fps(8,10),fpsSceneAirborne:fps(.1,1.5,'age'),at:[0,.1,.3,.6,1,1.5].map(t=>({target:t,...closest(t)})),wallAt:[.3,.6].map(t=>({target:t,...closest(t,'wallAge')}))};
 ...
 assert.deepEqual(errors,[]);await writeFile(`verify/grouping-${shots?'shots':'comparison'}.json`,JSON.stringify({results,errors},null,2));
