...
const profiles=(process.env.PROFILES||'launch').split(','),shots=process.argv.includes('--shots'),only=process.env.CASE;
...
try{
 for(const profile of profiles) {
  const dir=`verify/burst-fps-${profile==='legacy'?'before':profile==='launch'?'after':'current'}`;await mkdir(dir,{recursive:true});
 ...
 assert.deepEqual(errors,[]);await writeFile(`verify/burst-fps-${shots?'shots':'comparison'}.json`,JSON.stringify({results,errors},null,2));
