...
const result=JSON.parse(execFileSync('agent-browser',['--session','aquarium-self-shadow','--json','eval',js],{encoding:'utf8',maxBuffer:4e6})).data.result;
await writeFile('verify/lighting/self-shadow-sand.json',JSON.stringify(result,null,2));const {samples,...summary}=result;console.log(JSON.stringify(summary));
