...
export async function createLighting(scene: Scene, renderer: WebGPURenderer) {
  ...
  (sun.shadow as typeof sun.shadow & { filterNode: unknown }).filterNode = Fn(({depthTexture, shadowCoord}: any) => {
    ...
    for(let y=0;y<4;y++)for(let x=0;x<4;x++) {
      ...
      result.addAssign(texture(depthTexture,shadowCoord.xy.add(vec2(x-1.5,y-1.5).mul(2.2/3072))).compare(shadowCoord.z).mul(weight));
    }
    return result;
  }); sun.shadow.normalBias = 0.02; sun.shadow.bias = -0.00012;
  // Grazing receivers need extra depth clearance across the PCF footprint.
  sun.shadow.biasNode = float(-.00012).sub(float(1).sub(normalWorldGeometry.dot(lampOrigin.sub(positionWorld).normalize()).abs()).mul(.003));
