...
import './style.css';
export function createUI(actions: { shatter(): void; reset(): void; toggleTime(): void; toggleSound(): void }) {
  ...
  panel.innerHTML = `<div class="control-top"><span id="mode-label"><i></i> In equilibrium</span><span id="fps">— FPS</span></div><p>Click a wall to crack the glass · Space to shatter</p><div class="buttons"><button id="shatter"><span class="button-icon">✳</span> Shatter <kbd>SPACE</kbd></button><button id="slow" aria-pressed="false">Slow motion <kbd>T</kbd></button><button id="reset" aria-label="Reset aquarium">Reset <kbd>R</kbd></button><button id="sound" aria-pressed="true">Sound on <kbd>M</kbd></button></div><span class="sr-only">Click a wall to crack the glass · Space to shatter - Space: shatter - T: slow motion - R: reset</span>`;
