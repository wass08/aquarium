...
export function createReef(scene: Scene, terrain: Mesh) {
  ...
  geometries.forEach((geometry, kind) => {
    ...
    geometry.setAttribute('reefAnchor', new InstancedBufferAttribute(anchors, 3)); mesh.receiveShadow = true; mesh.castShadow = true; mesh.name = ['Coral fans', 'Tube coral', 'Brain coral', 'Seagrass'][kind]; scene.add(mesh);
  });
