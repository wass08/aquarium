...
import { Fn, If, mx_noise_float, color, float, mix, output, positionWorld, screenUV, smoothstep, uniform, vec2, vec3, vec4 } from 'three/tsl';
import { lampVisibility, causticCoordinates, causticParams, causticDrain, spotlightCone, lampAttenuation, towardLamp } from './lighting';
import { causticsField } from '../lib/worley';
import { lampOn, sunDirection, simTime, STAGE_Y, TANK, waterHeight, waterNormal, wetFootprints } from '../state';
...
export function createStage(scene: Scene, sun: SpotLight) {
  ...
  const pool = smoothstep(4.8, .8, distance).mul(positionWorld.xz.length().mul(-0.055).exp());
  // Backtrace through the tank bounds, clipped below the tilted surface. Overlap is in
  // world units; fading inward by .15 never leaks refracted light upstream.
  const ray=sunDirection.negate();
  const safe=(v:Node<'float'>)=>v.greaterThanEqual(0).select(v.max(.00001),v.min(-.00001));
  const direction=vec3(safe(ray.x),safe(ray.y),safe(ray.z));
  const a=vec3(-TANK.width/2+.06,TANK.floor,-TANK.depth/2+.06).sub(positionWorld).div(direction);
  const b=vec3(TANK.width/2-.06,TANK.top,TANK.depth/2-.06).sub(positionWorld).div(direction);
  const near=a.min(b),far=a.max(b);
  const entry=near.x.max(near.y).max(near.z).max(0),exit=far.x.min(far.y).min(far.z);
  const signed=positionWorld.sub(vec3(0,waterHeight,0)).dot(waterNormal),slope=direction.dot(waterNormal);
  const crossing=signed.negate().div(safe(slope));
  const wetEntry=slope.lessThan(-.00001).select(entry.max(crossing),entry),wetExit=slope.greaterThan(.00001).select(exit.min(crossing),exit);
  const waterHit=smoothstep(0,.15,wetExit.sub(wetEntry)).mul(slope.abs().lessThanEqual(.00001).select(signed.lessThanEqual(0).select(1,0),1)).mul(causticDrain);
  const floorColour = color('#041a2a').rgb.add(color('#0b4f78').mul(spotlightCone).mul(lampAttenuation).mul(smoothstep(4.8,1.6,distance)).mul(lampOn).mul(lampVisibility));
  ...
  const visibility = mix(float(1),lampVisibility,lampOn);
  ...
  material.colorNode = Fn(() => {
    ...
    If(pool.mul(amount).mul(waterHit).mul(lampOn).greaterThan(0.001), () => { light.assign(causticsField(causticCoordinates(.38), simTime, { ...causticParams, level: 3, depth: float(1) }).mul(color('#8fe3ff')).mul(pool).mul(amount).mul(waterHit).mul(lampOn).mul(.72).mul(spotlightCone)); });
    ...
    return shadowed.mul(float(1).sub(wet.mul(.12))).mul(rippleShade).add(light.mul(visibility)).min(vec3(1.20));
  })();
  ...
  return { update() { amount.value = Math.max(0, Math.min(1, (waterHeight.value - TANK.floor) / (TANK.base - TANK.floor))); }, get intensity() { return amount.value*lampOn.value; } };
}
