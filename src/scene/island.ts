import { DoubleSide, DataTexture, FloatType, RedFormat, LinearFilter, BoxGeometry, DodecahedronGeometry, InstancedMesh, Mesh, MeshStandardNodeMaterial, Object3D, Raycaster, Scene, Vector3 } from 'three/webgpu';
import { attribute, color, float, mix, mx_noise_float, positionWorld, smoothstep, vec3 } from 'three/tsl';
import { lampVisibility, causticLight, rockCaustics, signedWaterDistance, submerged, underwaterColor, waterDepth } from './lighting';
import { generateIslandTerrain } from '../lib/terrain';
import { lampOn, random, TANK } from '../state';
...
export function createIslandMaterial() {
  ...
  // This is an open height field: back-face-only shadow casting records the
  // far receiver itself and omits its lamp-facing occluding ridge.
  ...
  material.aoNode = mix(float(1), mix(mix(float(.50),float(.55),submerged),mix(float(1),float(.90),submerged),lampVisibility),lampOn);
  material.emissiveNode = rockCaustics.add(tint.rgb.mul(color('#b7d2e8')).mul(.050).mul(mix(float(.45),float(1),lampVisibility)).mul(submerged));
  return material;
}
export function createIsland(scene: Scene) {
  ...
  // Preserve the open-mesh caster side when the shadow diagnostic swaps materials.
  peak.onBeforeRender=(_renderer,_scene,_camera,_geometry,material)=>{material.shadowSide=DoubleSide;};
  ...
  // Occluders also attenuate the broad underwater fill at the sand contact.
  // Keep cool ambient when the lamp is off; this is not a second direct light.
  sand.aoNode=mix(float(1),mix(float(.70),float(.85),lampVisibility),lampOn);
  sand.emissiveNode = causticLight.mul(.65);
  ...
  gravelMaterial.colorNode = underwaterColor(color('#a19882').rgb); gravelMaterial.emissiveNode = causticLight.mul(.50);
  gravelMaterial.aoNode=mix(float(1),mix(float(.45),float(.90),lampVisibility),lampOn);
  ...
  return { peak, bed, focus: new Vector3(-.30,1.45,.15), heightTexture };
}
