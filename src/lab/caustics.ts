...
export function createCausticsBench({ root, camera, controls, pane, settings, level }: BenchContext): Bench {
  ...
  const params = Object.assign({ rawField: false, showSeeds: false, layer: 0, freeze: false, sineScaleA: 2.3, sineScaleB: 3.7, sineSpeed: 0.32, scaleA: 2.3, scaleB: 3.7, speed: 0.32, sharpness: 18, intensity: 1.65, rgbOffset: 0.012, waterLevel: 1.05 }, settings);
  ...
  const sine = { scaleA: uniform(params.sineScaleA), scaleB: uniform(params.sineScaleB), speed: uniform(params.sineSpeed) };
  const light = causticsField(projected, clock, { ...uniforms, ...(level === 1 ? sine : {}), level, depth });
  ...
  function sync() {
    sine.scaleA.value=params.sineScaleA; sine.scaleB.value=params.sineScaleB; sine.speed.value=params.sineSpeed;
  ...
  reveal.addBinding(params, 'rawField', { label: level === 1 ? 'raw sine field' : 'raw Worley field' });
  if (level !== 1) {
    reveal.addBinding(params, 'showSeeds', { label: 'cell seeds' });
    reveal.addBinding(params, 'layer', { options: { both: 0, A: 1, B: 2 } });
  }
  ...
  for (const [key, min, max, step] of [['scaleA', 0.5, 6, 0.1], ['scaleB', 0.5, 8, 0.1], ['speed', 0, 1, 0.01], ['sharpness', 2, 40, 1], ['intensity', 0, 4, 0.05], ['rgbOffset', 0, 0.05, 0.001], ['waterLevel', 0.2, 2, 0.01]] as const) {
    if (level === 1 && (key === 'sharpness' || key === 'intensity' || key === 'rgbOffset')) continue;
    const binding = level === 1 ? ({ scaleA: 'sineScaleA', scaleB: 'sineScaleB', speed: 'sineSpeed' } as const)[key as 'scaleA' | 'scaleB' | 'speed'] ?? key : key;
    folder.addBinding(params, binding, { min, max, step, label: level === 1 && key === 'scaleA' ? 'sine frequency A' : level === 1 && key === 'scaleB' ? 'sine frequency B' : key === 'rgbOffset' ? 'rgb offset' : key === 'waterLevel' ? 'water level' : key });
  }
