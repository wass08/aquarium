...
if (startedInLab) {
  if (/^#\/lab\/?(?:\?|$)/.test(location.hash)) history.replaceState(null, '', '#/lab/terrain?level=3');
  ...
  const link = document.createElement('a'); link.className = 'enter-lab'; link.href = '#/lab/terrain'; link.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M9 3h6m-5 0v7L4 19q-1 2 2 2h12q3 0 2-2l-6-9V3M8 15h8"/></svg> Enter the Lab'; document.body.append(link);
