...
function gridTriangulation(points: Point2[], segments: number) {
  const delaunay = delaunayFrom(points), triangles: number[] = [], stride = segments + 1;
  for (let z = 0; z < segments; z++) for (let x = 0; x < segments; x++) {
    const a = z * stride + x, b = a + 1, c = a + stride, d = c + 1;
    triangles.push(a, c, b, b, c, d);
  }
  delaunay.triangles = Uint32Array.from(triangles);
  delaunay.halfedges = new Int32Array(triangles.length).fill(-1);
  delaunay.inedges.fill(-1);
  const edges = new Map<string, number>();
  for (let i = 0; i < triangles.length; i++) {
    const a = triangles[i], b = triangles[i % 3 === 2 ? i - 2 : i + 1];
    const opposite = edges.get(`${b},${a}`);
    if (opposite !== undefined) { delaunay.halfedges[i] = opposite; delaunay.halfedges[opposite] = i; }
    edges.set(`${a},${b}`, i);
  }
  for (let i = 0; i < triangles.length; i++) {
    const b = triangles[i % 3 === 2 ? i - 2 : i + 1];
    if (delaunay.halfedges[i] === -1 || delaunay.inedges[b] === -1) delaunay.inedges[b] = i;
  }
  return delaunay;
}
